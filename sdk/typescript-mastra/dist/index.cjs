...
var enclave = require('@neuronedge/enclave');
...
// src/workspace.ts
var STATUS_NAMES = {
  [grpcJs.status.INVALID_ARGUMENT]: "INVALID_ARGUMENT",
  [grpcJs.status.NOT_FOUND]: "NOT_FOUND",
  [grpcJs.status.UNAVAILABLE]: "UNAVAILABLE",
  [grpcJs.status.DEADLINE_EXCEEDED]: "DEADLINE_EXCEEDED"
};
function mapServiceError(err) {
  if (enclave.isServiceError(err)) {
...
var messageOutputSchema = zod.z.object({ message: zod.z.string() });
var execInputSchema = zod.z.object({
  command: zod.z.string().describe("Path to the command binary, resolved against guest $PATH."),
  args: zod.z.array(zod.z.string()).default([]).describe("Arguments passed verbatim \u2014 no shell interpretation."),
  timeoutMs: zod.z.number().int().nonnegative().default(0).describe("Per-call guest timeout in milliseconds. 0 disables.")
});
...
function writeFileTool(client, workspaceId) {
  return tools.createTool({
    ...
    inputSchema: zod.z.object({
      ...
      content: zod.z.string().describe("File contents (written as UTF-8). Hard cap 10 MiB server-side.")
    }),
    outputSchema: messageOutputSchema,
...
function readFileTool(client, workspaceId) {
  return tools.createTool({
    ...
    inputSchema: zod.z.object({
      path: zod.z.string().describe("Relative path inside the workspace jail."),
      maxBytes: zod.z.number().int().nonnegative().default(0).describe("Max bytes to read. 0 uses the server default (10 MiB).")
    }),
    outputSchema: messageOutputSchema,
...
var EnclaveWorkspace = class {
  ...
  constructor(options) {
    this._opts = options;
    this._workspaceId = options.workspaceId ?? `agent-${crypto.randomUUID().replace(/-/g, "")}`;
  }
  ...
  async start() {
    ...
    if (this._opts._clientFactory) {
      ...
      client = new enclave.Client(clientOptions);
    }
    try {
...
exports.EnclaveWorkspace = EnclaveWorkspace;
exports.__version__ = __version__;
exports.createEnclaveTools = createEnclaveTools;
